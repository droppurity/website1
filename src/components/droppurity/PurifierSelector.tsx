...
export default function PurifierSelector({
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 mb-4 overflow-x-auto md:flex-wrap md:justify-center no-scrollbar pb-1 md:pb-0 px-1 md:px-0">
            ...
            {isSelected && purifier.tagline && (
              <div className="absolute -top-2 md:-top-2.5 left-1/2 -translate-x-1/2 z-10">
                <span className="inline-flex items-center px-2 py-0.5 md:px-2.5 rounded-full text-[10px] leading-tight md:text-xs font-semibold bg-primary text-primary-foreground shadow-md">
                  {TaglineIcon && <TaglineIcon className="w-2.5 h-2.5 md:w-3 md:h-3 mr-0.5 md:mr-1" />}
                  {purifier.tagline}
                </span>
              </div>
            )}
